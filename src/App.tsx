...
export default function VerificationDashboard() {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(() => {
    // Revisa si ya habíamos iniciado sesión antes
  ...
  // 🖼️ Pega aquí el enlace (URL) de tu logo.
  // Ejemplo: 'https://midominio.com/logo.png'
  // Si lo dejas vacío (''), aparecerá el candado azul por defecto.
  const LOGO_URL =
    "https://scontent.fbog2-4.fna.fbcdn.net/v/t39.30808-6/674956853_122171454344930844_2921257025913987444_n.jpg?_nc_cat=107&ccb=1-7&_nc_sid=13d280&_nc_ohc=PNV-tmLchfQQ7kNvwE1Iv5x&_nc_oc=Adq0w4Rv9rqun-_gAThrTIAxK2zvF4fsZz85CfgSQaQL4HuQI5qaWcvarYug6rn73LM&_nc_zt=23&_nc_ht=scontent.fbog2-4.fna&_nc_gid=KsmOWOtoT07UhfjWw5_DYA&_nc_ss=7a3a8&oh=00_Af1zUzGi5l9QfFRU24FE8ILkGrAnZlt4Qi-fan-HinFD8g&oe=69E8C730";
  // --------------------------------------------------------
  ...
  const handleLogin = (e) => {
    ...
    if (usernameInput === USER_CORRECTO && passwordInput === PASS_CORRECTA) {
      ...
      localStorage.setItem("authDashboard", "true"); // Guarda la sesión
      setLoginError("");
    } else {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("authDashboard"); // Borra la sesión
    setUsernameInput("");
    setPasswordInput("");
  };
  // -------------------------
  ...
  const [isDarkMode, setIsDarkMode] = useState(true); // Iniciamos en modo oscuro por defecto
  ...
  useEffect(() => {
    if (!isAuthenticated) return; // Solo conecta si está autenticado
    ...
    return () => unsubscribe();
  }, [isAuthenticated]); // Se vuelve a ejecutar si cambia el estado de autenticación
  ...
  const handleClearAll = async () => {
    setLoading(true);
    try {
      const deletePromises = codes.map((item) =>
        deleteDoc(doc(db, "received_codes", item.id))
      );
  ...
  const getServiceIcon = (service) => {
    switch (service) {
      case "Netflix":
        return <Tv className="w-5 h-5 text-red-600 dark:text-red-500" />;
      case "Disney+":
        return <Film className="w-5 h-5 text-blue-600 dark:text-blue-500" />;
      case "HBO":
        return (
            ...
            <div className="text-center mb-8">
              ...
              {LOGO_URL ? (
                <img
                  ...
                  onError={(e) => {
                    e.target.style.display = "none"; // Si la URL se rompe, la oculta
                  }}
                />
              ) : (
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900 p-4 md:p-8 font-sans transition-colors duration-200">
        <div className="max-w-5xl mx-auto space-y-6">
          <header className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 transition-colors">
            <div>
              <h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
                {/* Opcional: También podrías usar tu logo pequeño aquí al lado del título en vez del icono Inbox */}
          ...
          <div className="fixed top-4 right-4 z-50 flex flex-col gap-2">
            {notification && (
              <div
                className={`p-4 rounded-lg flex items-center gap-3 shadow-lg animate-fade-in-down transition-all ${
                  notification.type === "success"
                    ? "bg-green-50 dark:bg-green-900/50 text-green-800 dark:text-green-100 border border-green-200 dark:border-green-800"
                    : "bg-red-50 dark:bg-red-900/50 text-red-800 dark:text-red-100 border border-red-200 dark:border-red-800"
                }`}
              >
                {notification.type === "success" ? (
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col md:flex-row gap-4 transition-colors">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Busca el correo de tu cliente (ej. juan@hotmail.com)..."
                value={searchTerm}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden transition-colors">
            ...
            {filteredCodes.length === 0 ? (
              <div className="p-12 text-center text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm">
                  Conectado a tu Firebase. Esperando a que Make envíe el primer
                  código...
                </p>
              </div>
            ) : (
              <>
                <div className="divide-y divide-gray-100 dark:divide-slate-700/50">
                  {paginatedCodes.map((item) => {
                    // LIMPIEZA MÁGICA: Juntamos los números que vengan separados por espacios
                    ...
                    return (
                      <div
                        key={item.id}
                        className={`p-4 sm:p-6 transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${
                          item.status === "new"
                        ...
                        <div className="flex items-center gap-3 w-full sm:w-auto">
                          {item.type === "link" ? (
                            ...
                            <>
                              ...
                              <button
                                onClick={() =>
                                  copyToClipboard(cleanCode, item.id)
                                }
